...
- 🎯 Entry points to execute your program using `python -m <oitavo>` or `$ oitavo` with basic CLI argument parsing.
...
---
# oitavo Flask Application

Awesome oitavo created by PingNasa
...
```bash
git clone https://github.com/PingNasa/oitavo oitavo
cd oitavo
make install
```
...
```bash
oitavo create-db   # run once
oitavo populate-db  # run once (optional)
oitavo add-user -u admin -p 1234  # ads a user
oitavo run
```
